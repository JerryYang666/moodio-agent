...
import { generateAudioId, getPresignedAudioUploadUrl } from "@/lib/storage/s3";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!siteConfig.upload.allowedAudioTypes.includes(contentType)) {
      return NextResponse.json(
        { error: "Invalid file type. Supported: MP3, WAV" },
        { status: 400 }
      );
    }
    ...
    const uploadUrl = await getPresignedAudioUploadUrl(
      audioId,
      contentType,
      contentLength,
      expiresIn
    ...
    return NextResponse.json({ audioId, uploadUrl, expiresIn });
  } catch (error) {
