...
export default function ChatInterface({ chatId: initialChatId, initialMessages = [] }: ChatInterfaceProps) {
  ...
  const [isLoading, setIsLoading] = useState(!!initialChatId && initialMessages.length === 0);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (messages.length <= 1) { // checks length BEFORE this exchange is fully added to state logic wise, effectively 0 or 1 user message
