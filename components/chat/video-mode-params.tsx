...
export default function VideoModeParams({
  ...
  useEffect(() => {
    ...
    // multi_prompt) and hidden params that live in videoParams but are not controlled
    // by this component.
    const PRESERVED_TYPES = new Set(["media_references", "kling_elements", "multi_prompt"]);
    for (const param of model.params) {
      if (
        (PRESERVED_TYPES.has(param.type) || param.status === "hidden") &&
        videoParams[param.name] !== undefined
      ) {
