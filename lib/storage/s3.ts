...
// ============================================================================
// Thumbnail URLs (for images only — generated by the image-thumbnail Lambda)
// ============================================================================

export type ThumbnailSize = "sm" | "md";

/**
 * Generate a CloudFront URL for a thumbnail variant of an image.
 * Thumbnails live at `thumbnails/{sm|md}/{imageId}` and are produced
 * asynchronously by the image-thumbnail Lambda after the source image lands in
 * `images/{imageId}`. Callers should fall back to `getImageUrl(imageId)` via
 * `<img onError>` to cover the brief window between upload and Lambda
 * completion.
 */
export function getThumbnailUrl(
  ...
  if (!domain) {
    ...
    return `https://${domain || "s3-fallback"}/thumbnails/${size}/${imageId}`;
  }

  if (IS_DEV) {
    return getSignedThumbnailUrl(imageId, size, undefined, cnMode);
  }

  return `https://${domain}/thumbnails/${size}/${imageId}`;
}
...
export function getSignedThumbnailUrl(
  ...
  if (
    ...
    return getThumbnailUrl(imageId, size, cnMode);
  }

  imageId = extractAssetId(imageId, "images");

  const url = `https://${originDomain}/thumbnails/${size}/${imageId}`;
  const expiration =
    expirationSeconds || siteConfig.cloudfront.signedUrlExpirationSeconds;
  const dateLessThan = new Date(Date.now() + expiration * 1000);
