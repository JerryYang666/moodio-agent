...
5. For models with a "media_references" parameter (e.g., Seedance 2.0 Reference): pass an array of references, each with \`type\` ("image", "video", or "audio") and \`id\` (Image ID, Video ID, or Audio ID from the conversation). Reference them in the prompt using per-type numbering: images are numbered @image1, @image2, …; videos are numbered @video1, @video2, …; audios are numbered @audio1, @audio2, … — each type starts counting from 1 independently. Example: \`"media_references": [{"type": "video", "id": "vid1"}, {"type": "image", "id": "img1"}, {"type": "image", "id": "img2"}]\` → use @video1, @image1, @image2 in the prompt (NOT @video1, @image2, @image3).
