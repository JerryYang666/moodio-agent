...
- Each <VIDEO_SUGGEST> tag is self-contained. The JSON object must contain:
  ...
  - "referenceImageIds" (optional): An array of Image IDs from the conversation to use as reference images. Use this when the user asks you to create a video based on, inspired by, or building upon a previously generated or uploaded image from earlier in the conversation. Always try to include referenceImageIds when applicable — if a user mentions or refers to an earlier image, you MUST include its ID.
- If the user's input includes an image or references a previous image, make sure your prompts describe the video starting from that image as the first frame or key visual reference.`,
  examples: [
    ...
    `<VIDEO_SUGGEST>{"title": "Animate the portrait", "aspectRatio": "9:16", "prompt": "A close-up portrait of a woman with wind-swept auburn hair, soft golden-hour lighting on her face, a slight smile beginning to form, shallow depth of field with bokeh background", "videoIdea": "The woman's hair begins to flutter in a gentle breeze as she slowly turns toward the camera and breaks into a warm smile, with bokeh lights drifting softly behind her.", "referenceImageIds": ["abc123"]}</VIDEO_SUGGEST>`,
  ],
  waitForOutput: false,
  fireAndForget: true,
